...
const LineChart1 = ({ ssg1, isfetch, graphList }) => {
  ...
  const handleClick = (dataKey) => {
    setDisabled((prevDisabled) =>
      prevDisabled.includes(dataKey)
        ? prevDisabled.filter((key) => key !== dataKey)
        : [...prevDisabled, dataKey]
    );
  };
    ...
    if (active && payload && payload.length) {
      return (
        <div className="squaire-toolbox">
          ...
          >
            <Typography style={{ fontSize: "1em", color: "dimgrey" }}>
              {label}
            </Typography>
                  ...
                  return (
                    <tr key={"t" + i} style={{ fontSize: "0.8em" }}>
                      ...
                      <td style={{ color: v.color }}>
                        ...
                        {graph.value == "n" &&
                          `${Number(v.value * -1).toLocaleString()} ${
                            graph.unit
                          }`}
  ...
  return (
    ...
    >
      <Typography variant="h2" component="h2">
        {ssg1.def.tl1}の都道府県別の経年推移グラフ【{ssg1.def.tmn}〜
        {ssg1.def.tmx}】
      </Typography>
